    ...
    for (uint8_t i=1; i<SERIALMANAGER_NUM_PORTS; i++) {
        ...
        if (state[i].uart != nullptr) {
            ...
            switch (state[i].protocol) {
                ...
                case SerialProtocol_EFI_Fiala:
                    state[i].baud = AP_SERIALMANAGER_EFI_FIALA_BAUD;   // update baud param in case user looks at it
                    state[i].uart->begin(map_baudrate(state[i].baud),
                                         AP_SERIALMANAGER_EFI_FIALA_BUFSIZE_RX,
                                         AP_SERIALMANAGER_EFI_FIALA_BUFSIZE_TX);
                    state[i].uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                    break;
